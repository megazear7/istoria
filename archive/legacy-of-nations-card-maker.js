...
function printCard(fileIndex, path) {
    var fileName = "card-" + fileIndex + ".jpg";

    cardPaths[fileIndex-1] = activeDocument.path.fullName + "/dist/" + fileName;
...
function printSheet() {
    ...
    var path = activeDocument.path.fullName + "/dist/" + sheetName + ".jpg";
    var fileRef = new File(path);
    var sheetWidth = 10; // 4 cards wide = 10 inches
    var sheetHeight = 7; // 2 cards tall = 7 inches
    ...
    for (var i = 0; i < cardsPerSheet; i++) {
        if (cardPaths.length > i) {
            ...
            if (fileObj.exists) {
                placeFile(fileObj);
                fileObj.remove();
                var newLayer = sheetDoc.layers["card-" + (i + 1)];
